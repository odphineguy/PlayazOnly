...
export default function ManualDataEntry() {
  ...
  const addManualSeasonData = useMutation(api.manualData.addManualSeasonData);
  const getIncompleteSeasons = useQuery(api.manualData.getIncompleteSeasons, { 
  ...
  const handleSubmitSeasonData = async () => {
    ...
    try {
      const result = await addManualSeasonData({
        seasonId: "temp_season_id" as any,
        teams: [], // Would be populated from local state
        transactions: [],
        draftPicks: [],
      });
      toast.success(result.message);
